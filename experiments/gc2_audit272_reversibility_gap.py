    ...
    for y in Y:
        ...
        for x in X:
            ...
            for a,b in zip(x,y):
                ...
                else: ratios.append(Fraction(a,b))
            if feasible:
...
def omega(X,Y):
    m=M(X,Y); return INF if m is INF else max(Fraction(1),m)

def leq_mul(a,b,c):
    if b is INF or c is INF: return True
    if a is INF: return False
    return a<=b*c

def rg_zero(X,Y): return omega(X,Y)==1 and omega(Y,X)==1

def mink(A,B): return frozenset(tuple(x+y for x,y in zip(a,b)) for a in A for b in B)

def main():
    U=[(0,0),(0,1),(1,0),(1,1)]
    sets=[frozenset(c) for r in range(5) for c in combinations(U,r)]
    triples=0
    for X in sets:
      for Y in sets:
    ...
    for X in sets:
      for Y in sets:
        for s in [(2,3),(3,5)]:
            ...
            scale_checks+=1
    # Composition audit uses strictly positive coordinates so every dilation is finite;
    # zero-coordinate infinities are already covered by the exhaustive triangle audit.
    V=[(1,1),(1,2),(2,1),(2,2)]
    pos=[frozenset(c) for r in range(1,5) for c in combinations(V,r)]
    comp=0
    for X1 in pos:
      for Y1 in pos:
       for X2 in pos:
        for Y2 in pos:
            ...
            assert f<=max(f1,f2); assert r<=max(r1,r2)
            assert f*r<=(f1*r1)*(f2*r2)
            comp+=1
    print({"sets":len(sets),"triangle_triples":triples,
           "unit_rescaling_checks":scale_checks,"composition_checks":comp,
           "status":"PASS"})
