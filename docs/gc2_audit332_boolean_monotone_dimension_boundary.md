...
### Proof
Given a complete family, map `phi(x)={i : m_i(x)=1}`. Monotonicity gives `x<=y => phi(x) subseteq phi(y)`. Completeness gives the converse, so `phi` is an order embedding. Conversely, from any order embedding `phi:Q -> B_k`, each coordinate indicator `m_i(x)=1[i in phi(x)]` is monotone, and reflection of subset inclusion makes the coordinate family complete. Therefore the two minima coincide. QED.

This parameter is the classical **2-dimension / minimum inclusion-embedding dimension** of a poset. It must not be confused with the different modern parameter called **Boolean dimension**, which is defined using permutations plus a Boolean decoder. Status of the embedding mechanism: **IMPORTED/KNOWN**. Status of the GC-II identification: **PROVED**.

## Corollary 332.2 — universal information lower bound
If `q=|Q|`, then `bmd(Q) >= ceil(log2 q)` because `B_k` has only `2^k` elements. This lower bound is generally not tight because order structure, not only cardinality, constrains embeddings.
...
A longest chain in `B_k` has `k+1` elements, hence an embedding of `C_q` requires `k>=q-1`. Equality is attained by the prefix chain `emptyset subset {1} subset {1,2} subset ... subset {1,...,q-1}`. QED.
...
by Sperner's theorem. Hence two opposite order geometries behave very differently asymptotically:
...
A second tempting asymptotic claim, **maximal incomparability necessarily forces higher Boolean-coordinate complexity than total comparability**, is also **FALSIFIED**: antichains require only logarithmically many coordinates asymptotically, while chains require linearly many.
...
The scientifically stronger next question is not existence of complete monotones (classical), but the minimum dimension when coordinates must themselves be **operationally realizable, computable under a resource bound, local to an interface, or stable under composition**.

## Verification
`experiments/gc2_audit332_boolean_monotone_dimension_check.py` uses exact integer arithmetic to check explicit chain embeddings, the chain height obstruction, the Audit-331 Sperner threshold, and the cardinality lower bound for `q=1,...,64`.

## Status ledger
- Complete Boolean monotones <=> Boolean-lattice order embedding: **PROVED / 2-dimension mechanism IMPORTED-KNOWN**.
